...
describe('Phone Number Validation - Property Tests', () => {
    ...
    it('should reject phone numbers without + prefix', async () => {
        await fc.assert(fc.property(fc.integer({ min: 1, max: 9 }), fc.array(fc.integer({ min: 0, max: 9 }), { minLength: 1, maxLength: 14 }), (firstDigit, remainingDigits) => {
    ...
    it('should reject phone numbers with country code starting with 0', async () => {
        await fc.assert(fc.property(fc.array(fc.integer({ min: 0, max: 9 }), { minLength: 1, maxLength: 14 }), (digits) => {
    ...
    it('should reject phone numbers that are too short', async () => {
        await fc.assert(fc.property(fc.integer({ min: 1, max: 9 }), (digit) => {
    ...
    it('should reject phone numbers that are too long', async () => {
        await fc.assert(fc.property(fc.integer({ min: 1, max: 9 }), fc.array(fc.integer({ min: 0, max: 9 }), { minLength: 15, maxLength: 20 }), (firstDigit, remainingDigits) => {
    ...
    it('should reject phone numbers with non-digit characters', async () => {
        await fc.assert(fc.property(fc.string({ minLength: 5, maxLength: 15 }).filter(s => /[^0-9+]/.test(s)), (invalidString) => {
    ...
    it('should normalize phone numbers to E.164 format', async () => {
        await fc.assert(fc.property(fc.integer({ min: 1, max: 9 }), fc.array(fc.integer({ min: 0, max: 9 }), { minLength: 1, maxLength: 14 }), fc.constantFrom(' ', '-', '(', ')', '.', ''), (firstDigit, remainingDigits, separator) => {
    ...
    it('should be idempotent - normalizing twice equals normalizing once', async () => {
        await fc.assert(fc.property(fc.string({ minLength: 5, maxLength: 20 }), (phoneNumber) => {
