...
describe('Database Schema Integrity - Property Tests', () => {
    ...
    it('should maintain exact consistency after contact synchronization', async () => {
        await fc.assert(fc.asyncProperty(contactListArbitrary, async (externalContacts) => {
            const client = await pool.connect();
            try {
                await client.query('BEGIN');
                // Create a test user
                const userResult = await client.query(`INSERT INTO users (email, cognito_user_id, role) 
             VALUES ($1, $2, $3) RETURNING id`, [`test_user_${Date.now()}@example.com`, `cognito_${Date.now()}`, 'CampaignManager']);
                const userId = userResult.rows[0].id;
                // Create a test campaign
                const campaignResult = await client.query(`INSERT INTO campaigns (name, type, status, config, created_by) 
             VALUES ($1, $2, $3, $4, $5) RETURNING id`, [
                ...
                for (const contact of externalContacts) {
                    const result = await client.query(`INSERT INTO contacts (campaign_id, phone_number, metadata, timezone, sms_capable)
               VALUES ($1, $2, $3, $4, $5)
               ...
               RETURNING id, phone_number, metadata, timezone, sms_capable`, [
                ...
                const queryResult = await client.query(`SELECT id, phone_number, metadata, timezone, sms_capable
             FROM contacts
             WHERE campaign_id = $1
    ...
    it('should enforce foreign key constraints for campaign references', async () => {
        await fc.assert(fc.asyncProperty(contactRecordArbitrary, async (contact) => {
            const client = await pool.connect();
            try {
                await client.query('BEGIN');
                ...
                await expect(client.query(`INSERT INTO contacts (campaign_id, phone_number, metadata, timezone, sms_capable)
               VALUES ($1, $2, $3, $4, $5)`, [
    ...
    it('should prevent duplicate phone numbers within the same campaign', async () => {
        await fc.assert(fc.asyncProperty(phoneNumberArbitrary, async (phoneNumber) => {
            const client = await pool.connect();
            try {
                await client.query('BEGIN');
             ...
             VALUES ($1, $2, $3, $4, $5) RETURNING id`, [
                ...
                await client.query(`INSERT INTO contacts (campaign_id, phone_number)
             VALUES ($1, $2)`, [campaignId, phoneNumber]);
                // Attempt to insert duplicate contact - should fail
                await expect(client.query(`INSERT INTO contacts (campaign_id, phone_number)
               VALUES ($1, $2)`, [campaignId, phoneNumber])).rejects.toThrow();
                await client.query('ROLLBACK');
            }
            catch (error) {
                await client.query('ROLLBACK');
